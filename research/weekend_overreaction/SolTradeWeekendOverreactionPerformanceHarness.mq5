...
#property description "V27 tester-only multi-currency weekend-overreaction performance harness"
...
input string DatasetId="V27_2025_DEVELOPMENT";
input string ExecutionLayer="NATIVE_NORMAL_EXECUTION";
input int ExpectedExecutionMode=0;
input int ExpectedScheduleSignals=45;
input string ScheduleFile="SolTrade\\Phase6\\V27Signals\\signal-ledger.csv";
input string ExecutionInstanceId="V27-2025-NATIVE";
input string OutputRoot="SolTrade\\Phase6\\V27Performance\\V27-2025-NATIVE";

#define V27_MAGIC_BASE 2708202600
...
struct V27Signal {datetime target;datetime exit_time;int symbol_index;string direction;string tail;double gap;};
V27Signal g_signals[];
...
datetime g_exit_target[7],g_max_tick=0;
...
void Event(string type,string reason,string detail=""){if(g_events!=INVALID_HANDLE)FileWrite(g_events,"SOLTRADE_PHASE6_V27_EVENT_V1",TS(TimeCurrent()),DatasetId,ExecutionLayer,type,reason,detail);}

void LoadConfig(int i)
  {
   g_config[i].strategy_version="FX_WEEKEND_OVERREACTION_REVERSAL_5PCT_1W_1_0";g_config[i].approved_strategy_version="";g_config[i].risk_profile="CONSERVATIVE_V1";g_config[i].approved_risk_profile="";g_config[i].magic_number=V27_MAGIC_BASE+i+1;g_config[i].symbol=SYMBOLS[i];g_config[i].timeframe=PERIOD_H1;g_config[i].minimum_history_bars=300;g_config[i].max_tick_age_seconds=120;g_config[i].max_spread_points=30;g_config[i].max_spread_atr_percent=99.0;g_config[i].max_slippage_points=10;g_config[i].risk_per_trade_percent=.5;g_config[i].daily_loss_limit_percent=1.0;g_config[i].weekly_loss_limit_percent=2.5;g_config[i].emergency_drawdown_percent=5.0;g_config[i].production_baseline_equity=0;g_config[i].consecutive_loss_limit=3;g_config[i].reset_emergency_lock=false;g_config[i].expected_environment=SOLTRADE_ENV_BACKTEST;g_config[i].enable_demo_execution=false;g_config[i].enable_position_management=false;g_config[i].approved_demo_account=0;g_config[i].allow_live_trading=false;g_config[i].approved_live_account=0;g_config[i].emergency_stop=false;g_config[i].enable_backtest_research=true;g_config[i].enable_backtest_execution=true;g_config[i].enable_backtest_position_management=true;g_config[i].research_manifest_id="PHASE6-V27-WEEKEND-OVERREACTION";g_config[i].execution_instance_id=ExecutionInstanceId+"-"+SYMBOLS[i];g_config[i].research_dataset=SOLTRADE_DATASET_DEVELOPMENT;g_config[i].research_cost_profile=SOLTRADE_COST_NORMAL;g_config[i].research_start_inclusive=EligibleFrom;g_config[i].research_end_exclusive=EligibleTo;g_config[i].research_history_fingerprint="daa8484b47e1a1792a8d439181ffe9c4c2f9eef527252f532cb81bdab2025547";g_config[i].research_latency_fingerprint="e301e77895e8f095d485b00bd1b5da9f10f07d6c8b6ae9162048a7643ddf4dfe";g_config[i].research_latency_sample_count=30;g_config[i].research_frozen_delay_ms=200;g_config[i].research_source_commit="782a4e5b7b93be127fac662ca1af09a22510e772";g_config[i].research_build_fingerprint="105db9cb0275d2f386e75bd3786c37077c6299073c9cba15fdff68100a3ba706";g_config[i].research_expected_terminal_build=6090;g_config[i].research_expected_broker_server="FPMarketsSC-Demo";g_config[i].research_expected_initial_deposit=10000;g_config[i].research_expected_deposit_currency="USD";g_config[i].research_expected_leverage=30;g_config[i].research_expected_trading_input_hash="105db9cb0275d2f386e75bd3786c37077c6299073c9cba15fdff68100a3ba706";g_config[i].research_state_root="SolTradeBot\\phase6-v27-state\\"+ExecutionInstanceId+"\\"+SYMBOLS[i];g_config[i].research_artifact_root="SolTradeBot\\phase6-v27-artifacts\\"+ExecutionInstanceId+"\\"+SYMBOLS[i];g_config[i].enable_csv_journal=true;g_config[i].journal_directory="SolTradeBot\\phase6-v27-journal\\"+ExecutionInstanceId+"\\"+SYMBOLS[i];g_config[i].risk_state_directory="SolTradeBot\\phase6-v27-risk\\"+ExecutionInstanceId;g_config[i].execution_state_directory=g_config[i].research_state_root;g_config[i].enable_dashboard=false;g_config[i].dashboard_refresh_seconds=1;
  }
...
bool LoadSchedule(string &reason)
  {
   ...
   while(!FileIsEnding(h))
     {
      string schema=FileReadString(h);if(schema=="")break;string dataset=FileReadString(h);FileReadString(h);string symbol=FileReadString(h);int orientation=(int)StringToInteger(FileReadString(h));string status=FileReadString(h),tail=FileReadString(h);double gap=StringToDouble(FileReadString(h));FileReadString(h);FileReadString(h);FileReadString(h);FileReadString(h);FileReadString(h);datetime target=StringToTime(FileReadString(h));datetime exit_time=StringToTime(FileReadString(h));
      if(dataset!=DatasetId||status!="EVALUATED"||(tail!="UPPER"&&tail!="LOWER"))continue;int index=SymbolIndex(symbol);if(index<0){FileClose(h);reason="UNKNOWN_SCHEDULE_SYMBOL";return false;}
      int n=ArraySize(g_signals);ArrayResize(g_signals,n+1,128);g_signals[n].target=target;g_signals[n].exit_time=exit_time;g_signals[n].symbol_index=index;g_signals[n].tail=tail;g_signals[n].gap=gap;
      bool foreign_buy=(tail=="LOWER");g_signals[n].direction=((foreign_buy==(orientation>0)))?"BUY":"SELL";
     }
...
int PortfolioPositions(){int count=0;for(int p=0;p<PositionsTotal();p++){ulong ticket=PositionGetTicket(p);if(ticket==0)continue;long magic=PositionGetInteger(POSITION_MAGIC);if(magic>V27_MAGIC_BASE&&magic<=V27_MAGIC_BASE+7)count++;}return count;}
...
int OnInit()
  {
   ...
   for(int i=0;i<7;i++){g_exit_target[i]=0;g_atr[i]=INVALID_HANDLE;ResetSolTradeMarketSnapshot(g_market[i]);ResetSolTradeExecutionStatus(g_execution[i]);ResetSolTradePositionStatus(g_position[i]);EvaluateSolTradeAccountSafety(g_config[i],g_account[i]);datetime last=0;if(!InitialiseSolTradeMarketData(g_config[i],last,reason)||!g_exec[i].Initialise(g_config[i],g_account[i].account_identifier_hash,reason)||!g_pm[i].Initialise(g_config[i],g_account[i].account_identifier_hash,reason)){Print("SOLTRADE_V27_ENGINE_INIT_FAILED | ",SYMBOLS[i]," | ",reason);return INIT_FAILED;}g_atr[i]=iATR(SYMBOLS[i],PERIOD_D1,14);if(g_atr[i]==INVALID_HANDLE)return INIT_FAILED;}
   if(!g_risk_engine.Initialise(g_config[0],g_account[0].account_identifier_hash,TimeCurrent(),AccountInfoDouble(ACCOUNT_EQUITY),reason)){Print("SOLTRADE_V27_RISK_INIT_FAILED | ",reason);return INIT_FAILED;}ResetSolTradeRiskStatus(g_risk);g_preflight=true;Event("PREFLIGHT_PASSED","NONE","tester_only=YES;optimization=NO;symbols=7");return INIT_SUCCEEDED;
  }
   ...
   if(TimeCurrent()>=EligibleTo)return;for(int i=0;i<7;i++)
     {
      ...
      SolTradePositionReport exit;if(g_pm[i].HandleTradeTransaction(t,exit)){if(exit.fill_confirmed){g_exit_fills++;g_exit_target[i]=0;}Event(exit.event_type,exit.reason_code,SYMBOLS[i]+";"+exit.exit_reason_code);RecordExit("EXIT_TRANSACTION",i,exit);string reason="";g_risk_engine.RecordClosedOutcome("V27_EXIT_"+StringFormat("%I64u",exit.deal_ticket),exit.final_profit_loss,TimeCurrent(),AccountInfoDouble(ACCOUNT_EQUITY),reason);g_risk_engine.GetStatus(g_risk);}
     }
  }
double OnTester(){bool ok=WriteDeals()&&WriteSummary();if(g_events!=INVALID_HANDLE)FileFlush(g_events);if(g_transactions!=INVALID_HANDLE)FileFlush(g_transactions);Print("SOLTRADE_V27_RUN_RESULT | status=",ok?"PASS":"FAIL"," | dataset=",DatasetId," | layer=",ExecutionLayer," | entries=",g_entry_fills," | exits=",g_exit_fills," | open=",PortfolioPositions());return ok?1.0:0.0;}

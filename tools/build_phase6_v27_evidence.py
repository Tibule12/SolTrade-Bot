#!/usr/bin/env python3
"""Reconcile V27 deals, construct twelve formal cells, and apply frozen gates."""
...
BASE=OUT/"performance-runs-attempt-3"
FMT="%Y.%m.%d %H:%M:%S"
SPANS={"V27_2025_DEVELOPMENT":(datetime(2025,1,6),datetime(2026,1,1)),"V27_2026_PRESEAL_DEVELOPMENT":(datetime(2026,1,5),datetime(2026,8,1))}
...
def main():
 ...
 with (OUT/"phase6-v27-complete-adjusted-trade-ledger.csv").open("w",newline="") as h:w=csv.DictWriter(h,fieldnames=LEDGER_FIELDS,extrasaction="ignore",lineterminator="\n");w.writeheader();w.writerows(ledger)
 with (OUT/"phase6-v27-formal-cell-metrics.csv").open("w",newline="") as h:w=csv.DictWriter(h,fieldnames=list(formal[0]),lineterminator="\n");w.writeheader();w.writerows(formal)
 auth=[r for r in ledger if r["cost_profile"]=="NORMAL" and r["execution_layer"]=="NATIVE_NORMAL_EXECUTION"]
 sample={"all_preseal":len(auth),"dated_2026":sum(r["dataset"]=="V27_2026_PRESEAL_DEVELOPMENT" for r in auth),"BUY":sum(r["direction"]=="BUY" for r in auth),"SELL":sum(r["direction"]=="SELL" for r in auth),"by_symbol":{s:sum(r["symbol"]==s for r in auth) for s in SYMBOLS}}
 sg=[("all_preseal_naturally_closed >= 50",sample["all_preseal"]>=50,sample["all_preseal"]),("dated_2026_naturally_closed >= 15",sample["dated_2026"]>=15,sample["dated_2026"]),("BUY_naturally_closed >= 20",sample["BUY"]>=20,sample["BUY"]),("SELL_naturally_closed >= 20",sample["SELL"]>=20,sample["SELL"])]
 sg += [(f"{s}_naturally_closed >= 4",n>=4,n) for s,n in sample["by_symbol"].items()]
 ...
 outcome="V27_DEVELOPMENT_GATES_PASSED_DEMO_AUTHORIZED" if all(x["status"]=="PASS" for x in sample_gates+pg+cg) else "V27_DEVELOPMENT_GATES_FAILED_CANDIDATE_RETIRED"
 write("phase6-v27-gate-evaluation.json",{"schema":"SOLTRADE_PHASE6_V27_GATE_EVALUATION_V1","terminal_outcome":outcome,"sample_summary":sample,"sample_gates":sample_gates,"performance_gates":pg,"consistency_gates":cg})
 write("phase6-v27-formal-cell-inventory.json",{"schema":"SOLTRADE_PHASE6_V27_FORMAL_CELL_INVENTORY_V1","status":"PASS","cells":formal})
 write("phase6-v27-cross-dataset-consistency.json",{"schema":"SOLTRADE_PHASE6_V27_CROSS_DATASET_CONSISTENCY_V1","groups":groups,"gates":cg})
 write("phase6-v27-physical-run-inventory.json",{"schema":"SOLTRADE_PHASE6_V27_PHYSICAL_RUN_INVENTORY_V1","status":"PASS","valid_runs":4,"technical_prefight_failures_retained":2,"valid_result_reruns":0,"runs":[{"run_id":r["run_id"],"entry_fills":len(info[r["run_id"]]["rows"]),"closed_positions":len(info[r["run_id"]]["rows"]),"seal_breach":info[r["run_id"]]["summary"]["seal_breach"]} for r in RUNS]})
 write("phase6-v27-commission-reconciliation.json",{"schema":"SOLTRADE_PHASE6_V27_COMMISSION_RECONCILIATION_V1","status":"PASS","native_tester_commission_total":sum(r["native_tester_commission"] for r in ledger),"external_usd_per_side_per_standard_lot":3,"double_counted":False})
 raw=[]
 for ds in SPANS:
 ...
 write("phase6-v27-before-external-cost-check.json",{"schema":"SOLTRADE_PHASE6_V27_BEFORE_EXTERNAL_COST_CHECK_V1","datasets":raw})
 seed=sha(OUT/"candidate-strategy-specification.json")+sha(OUT/"physical-run-plan.json");vals=[r["adjusted_net_R"] for r in sorted(auth,key=lambda r:(r["exit_time"],r["symbol"]))];bs=int(hashlib.sha256((seed+":bootstrap").encode()).hexdigest()[:16],16);ms=int(hashlib.sha256((seed+":monte").encode()).hexdigest()[:16],16);b=uncertainty(vals,bs,True);b.update({"schema":"SOLTRADE_PHASE6_V27_BOOTSTRAP_V1","seed":bs,"reporting_only":True});write("phase6-v27-bootstrap-report.json",b);m=uncertainty(vals,ms,False);m.update({"schema":"SOLTRADE_PHASE6_V27_MONTE_CARLO_V1","seed":ms,"reporting_only":True});write("phase6-v27-monte-carlo-report.json",m)
 prod=sha(ROOT/"MQL5/Experts/SolTradeBot.mq5");write("phase6-v27-evidence-integrity.json",{"schema":"SOLTRADE_PHASE6_V27_EVIDENCE_INTEGRITY_V1","status":"PASS","physical_runs":4,"formal_cells":12,"technical_preflight_failures_retained":2,"valid_result_reruns":0,"optimization_or_tuning":False,"post_seal_data_accessed":False,"production_phase1_5_unchanged":prod=="261a9cfe1c1e8d84e2a2a468ac4d0775086b21c89824b117e5127697fd03ced3","connected_chart_trades":0,"demo_trades":0,"live_trades":0,"terminal_outcome":outcome})
 nn=[x for x in formal if x["cost_profile"]=="NORMAL" and x["execution_layer"]=="NATIVE_NORMAL_EXECUTION"]
 (OUT/"phase6-v27-terminal-outcome.md").write_text(f"# Phase 6 V27 terminal outcome\n\n`{outcome}`\n\nAll four frozen real-tick runs and twelve formal cells are valid. The authoritative sample contains {sample['all_preseal']} closed trades, including {sample['dated_2026']} in 2026, {sample['BUY']} BUY and {sample['SELL']} SELL.\n\nNormal/Native 2025: PF {nn[0]['profit_factor']:.4f}, net USD {nn[0]['adjusted_net_profit']:.2f}, expectancy {nn[0]['expectancy_R']:.6f} R, drawdown {nn[0]['relative_drawdown_percent']:.4f}%. Normal/Native 2026: PF {nn[1]['profit_factor']:.4f}, net USD {nn[1]['adjusted_net_profit']:.2f}, expectancy {nn[1]['expectancy_R']:.6f} R, drawdown {nn[1]['relative_drawdown_percent']:.4f}%. The 2026 loss was present before external commission, so costs did not create the failure.\n\nV27 is retired unchanged. No optimization, sealed-OOS access, demo trade, or live trade occurred. Phase 1-5 production code remains unchanged.\n")
 checksum=OUT/"artifact-sha256-v27.txt";arts=sorted(p for p in OUT.rglob("*") if p.is_file() and p!=checksum);checksum.write_text("".join(f"{sha(p)}  {p.relative_to(OUT).as_posix()}\n" for p in arts));print(json.dumps({"outcome":outcome,"sample":sample,"normal_native":nn,"performance_failed":sum(x["status"]=="FAIL" for x in pg),"consistency_failed":sum(x["status"]=="FAIL" for x in cg)},indent=2))

...
#property description "V30 date-bounded signal-only replication of frozen V28 rules"

input datetime EligibleFrom=D'2018.01.01 00:00:00';
input datetime EligibleTo=D'2025.01.01 00:00:00';
input string DatasetId="V30_2018_2024_HISTORICAL";
input string OutputRoot="SolTrade\\Phase6\\V30Signals";

string SYMBOLS[7]={"EURUSD","GBPUSD","AUDUSD","NZDUSD","USDCAD","USDCHF","USDJPY"};
int ORIENTATION[7]={1,1,1,1,-1,-1,-1};
int g_file=INVALID_HANDLE,g_cohorts=0,g_legs=0,g_buy=0,g_sell=0,g_skips=0,g_equalities=0,g_key=0;
datetime g_max=0;bool g_boundary_breach=false,g_ok=false;
...
void Evaluate(datetime monday)
  {
   ...
   if(target<EligibleFrom||exit_time>=EligibleTo)return;
   datetime recent=monday+9*3600,anchor=PreviousFirstMonday(monday)+9*3600;
   double returns[7],rc[7],ac[7],factor=0;
   for(int i=0;i<7;i++)
     {
      if(!ExactClose(SYMBOLS[i],recent,rc[i])||!ExactClose(SYMBOLS[i],anchor,ac[i])){g_skips++;return;}
      returns[i]=ORIENTATION[i]*MathLog(rc[i]/ac[i]);factor+=returns[i]/7.0;
     }
   if(factor==0){g_equalities++;return;}
   bool foreign_buy=factor>0;
   for(int i=0;i<7;i++)
     {
      string direction=(foreign_buy==(ORIENTATION[i]>0))?"BUY":"SELL";
      if(direction=="BUY")g_buy++;else g_sell++;g_legs++;
      FileWrite(g_file,"SOLTRADE_PHASE6_V28_SIGNAL_V1",DatasetId,TS(target),SYMBOLS[i],ORIENTATION[i],DoubleToString(factor,12),foreign_buy?"LONG_FOREIGN_FACTOR":"SHORT_FOREIGN_FACTOR",direction,TS(recent),DoubleToString(rc[i],10),TS(anchor),DoubleToString(ac[i],10),TS(exit_time));
     }
   g_cohorts++;
  }
...
int OnInit()
  {
   if(!MQLInfoInteger(MQL_TESTER)||MQLInfoInteger(MQL_OPTIMIZATION)||(int)TerminalInfoInteger(TERMINAL_BUILD)!=6090||AccountInfoString(ACCOUNT_SERVER)!="FPMarketsSC-Demo"||EligibleFrom!=D'2018.01.01 00:00:00'||EligibleTo!=D'2025.01.01 00:00:00'||DatasetId!="V30_2018_2024_HISTORICAL")return INIT_PARAMETERS_INCORRECT;
   ...
   FileWrite(g_file,"schema","dataset","target","symbol","orientation","dollar_factor_return","factor_side","chart_direction","recent_h1","recent_close","anchor_h1","anchor_close","scheduled_exit");
   g_ok=true;return INIT_SUCCEEDED;
  }
void OnTick()
  {
   datetime now=TimeCurrent();if(now>=EligibleTo){g_boundary_breach=true;return;}g_max=now;
...
bool Summary()
  {
   ...
   FileWrite(h,"field","value");FileWrite(h,"schema","SOLTRADE_PHASE6_V30_SIGNAL_SUMMARY_V1");FileWrite(h,"cohorts",g_cohorts);FileWrite(h,"legs",g_legs);FileWrite(h,"buy",g_buy);FileWrite(h,"sell",g_sell);FileWrite(h,"missing_data_skips",g_skips);FileWrite(h,"equality_skips",g_equalities);FileWrite(h,"max_tick",TS(g_max));FileWrite(h,"boundary_breach",g_boundary_breach?"YES":"NO");FileWrite(h,"orders_or_positions","ZERO");FileWrite(h,"pnl_calculated","NO");
   bool pass=g_ok&&!g_boundary_breach&&g_skips==0&&g_cohorts==77&&g_legs==539;FileWrite(h,"status",pass?"PASS":"FAIL");FileClose(h);return pass;
  }

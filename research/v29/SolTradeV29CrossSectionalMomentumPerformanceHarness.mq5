...
#property description "V29 tester-only frozen-V26 weekly cross-sectional momentum performance harness"
...
input string DatasetId="V29_2025_DEVELOPMENT";
input string ScheduleDatasetId="V26_2025_DEVELOPMENT";
input string ExecutionLayer="NATIVE_NORMAL_EXECUTION";
input int ExpectedExecutionMode=0;
input int ExpectedScheduleLegs=204;
input string ScheduleFile="SolTrade\\Phase6\\V29Signals\\V26_2025_DEVELOPMENT\\signal-schedule.csv";
input string ExecutionInstanceId="V29-2025-NATIVE";
input string OutputRoot="SolTrade\\Phase6\\V29Performance\\V29-2025-NATIVE";

#define V29_MAGIC_BASE 2908202600
...
struct V29Leg {datetime target;datetime exit_time;int symbol_index;string direction;string portfolio_side;};
V29Leg g_legs[];
int g_leg_count=0,g_next=0,g_rebalances=0,g_entry_attempts=0,g_entry_fills=0,g_exit_fills=0,g_missed=0,g_spread_blocks=0,g_risk_blocks=0,g_execution_blocks=0,g_max_portfolio_positions=0;
datetime g_max_tick=0,g_close_target=0,g_last_exit_submission_tick=0,g_final_exit_target=0;
...
void Event(string type,string reason,string detail=""){if(g_events!=INVALID_HANDLE)FileWrite(g_events,"SOLTRADE_PHASE6_V29_EVENT_V1",TS(TimeCurrent()),DatasetId,ExecutionLayer,type,reason,detail);}

void LoadConfig(int i)
  {
   g_config[i].strategy_version="FX_CROSS_SECTIONAL_MOMENTUM_3W_1W_TOP2_BOTTOM2_V29_1_0";g_config[i].approved_strategy_version="";g_config[i].risk_profile="CONSERVATIVE_V1";g_config[i].approved_risk_profile="";g_config[i].magic_number=V29_MAGIC_BASE+i+1;g_config[i].symbol=SYMBOLS[i];g_config[i].timeframe=PERIOD_H1;g_config[i].minimum_history_bars=300;g_config[i].max_tick_age_seconds=120;g_config[i].max_spread_points=30;g_config[i].max_spread_atr_percent=99.0;g_config[i].max_slippage_points=10;g_config[i].risk_per_trade_percent=.25;g_config[i].daily_loss_limit_percent=1.0;g_config[i].weekly_loss_limit_percent=2.5;g_config[i].emergency_drawdown_percent=5.0;g_config[i].production_baseline_equity=0;g_config[i].consecutive_loss_limit=3;g_config[i].reset_emergency_lock=false;g_config[i].expected_environment=SOLTRADE_ENV_BACKTEST;g_config[i].enable_demo_execution=false;g_config[i].enable_position_management=false;g_config[i].approved_demo_account=0;g_config[i].allow_live_trading=false;g_config[i].approved_live_account=0;g_config[i].emergency_stop=false;g_config[i].enable_backtest_research=true;g_config[i].enable_backtest_execution=true;g_config[i].enable_backtest_position_management=true;g_config[i].research_manifest_id="PHASE6-V29-INDEPENDENT-CROSS-SECTIONAL-MOMENTUM";g_config[i].execution_instance_id=ExecutionInstanceId+"-"+SYMBOLS[i];g_config[i].research_dataset=SOLTRADE_DATASET_DEVELOPMENT;g_config[i].research_cost_profile=SOLTRADE_COST_NORMAL;g_config[i].research_start_inclusive=EligibleFrom;g_config[i].research_end_exclusive=EligibleTo;g_config[i].research_history_fingerprint="7e437783c8e9b5b616bd301c4a0434afa208b2339fdc80064ad4675425014db9";g_config[i].research_latency_fingerprint="e301e77895e8f095d485b00bd1b5da9f10f07d6c8b6ae9162048a7643ddf4dfe";g_config[i].research_latency_sample_count=30;g_config[i].research_frozen_delay_ms=200;g_config[i].research_source_commit="51216f893b0774239fe29bbdd24408dfd7f86da7";g_config[i].research_build_fingerprint="7a179da0edd09d93556ac402641d666c37cf3d842e4e497f313ed186ad21414f";g_config[i].research_expected_terminal_build=6090;g_config[i].research_expected_broker_server="FPMarketsSC-Demo";g_config[i].research_expected_initial_deposit=10000;g_config[i].research_expected_deposit_currency="USD";g_config[i].research_expected_leverage=30;g_config[i].research_expected_trading_input_hash="7a179da0edd09d93556ac402641d666c37cf3d842e4e497f313ed186ad21414f";g_config[i].research_state_root="SolTradeBot\\phase6-v29-state\\"+ExecutionInstanceId+"\\"+SYMBOLS[i];g_config[i].research_artifact_root="SolTradeBot\\phase6-v29-artifacts\\"+ExecutionInstanceId+"\\"+SYMBOLS[i];g_config[i].enable_csv_journal=true;g_config[i].journal_directory="SolTradeBot\\phase6-v29-journal\\"+ExecutionInstanceId+"\\"+SYMBOLS[i];g_config[i].risk_state_directory="SolTradeBot\\phase6-v29-risk\\"+ExecutionInstanceId;g_config[i].execution_state_directory=g_config[i].research_state_root;g_config[i].enable_dashboard=false;g_config[i].dashboard_refresh_seconds=1;
  }
   ...
   while(!FileIsEnding(h))
     {
      ...
      if(dataset!=ScheduleDatasetId||direction=="NONE")continue;
...
bool Preflight(string &reason)
  {
   ...
   if((DatasetId=="V29_2025_DEVELOPMENT"&&(ScheduleDatasetId!="V26_2025_DEVELOPMENT"||ExpectedScheduleLegs!=204))||(DatasetId=="V29_2026_PRESEAL_DEVELOPMENT"&&(ScheduleDatasetId!="V26_2026_PRESEAL_DEVELOPMENT"||ExpectedScheduleLegs!=116))||(DatasetId!="V29_2025_DEVELOPMENT"&&DatasetId!="V29_2026_PRESEAL_DEVELOPMENT")){reason="FROZEN_DATASET_SCHEDULE_MAPPING_INVALID";return false;}
...
int PortfolioPositions()
  {int count=0;for(int p=0;p<PositionsTotal();p++){ulong ticket=PositionGetTicket(p);if(ticket==0)continue;long magic=PositionGetInteger(POSITION_MAGIC);if(magic>V29_MAGIC_BASE&&magic<=V29_MAGIC_BASE+7)count++;}return count;}
...
void RecordEntry(string record_type,int index,datetime target,SolTradeExecutionReport &r,double atr)
  {if(g_transactions==INVALID_HANDLE)return;FileWrite(g_transactions,"SOLTRADE_PHASE6_V29_TRANSACTION_V1",TS(TimeCurrent()),DatasetId,ExecutionLayer,SYMBOLS[index],record_type,TS(target),r.signal_result,DoubleToString(r.requested_entry,10),DoubleToString(r.actual_entry,10),r.spread_points,DoubleToString(r.slippage_points,4),DoubleToString(r.volume,8),DoubleToString(r.risk_amount,8),DoubleToString(r.stop_loss,10),StringFormat("%I64u",r.order_ticket),StringFormat("%I64u",r.deal_ticket),(int)r.broker_return_code,r.fill_confirmed?"YES":"NO",DoubleToString(atr,12));}
void RecordExit(string record_type,int index,SolTradePositionReport &r)
  {if(g_transactions==INVALID_HANDLE)return;FileWrite(g_transactions,"SOLTRADE_PHASE6_V29_TRANSACTION_V1",TS(TimeCurrent()),DatasetId,ExecutionLayer,SYMBOLS[index],record_type,TS(r.signal_bar_time),r.position_direction,DoubleToString(r.requested_close_price,10),DoubleToString(r.actual_close_price,10),0,DoubleToString(r.slippage_points,4),DoubleToString(r.volume,8),0,0,StringFormat("%I64u",r.order_ticket),StringFormat("%I64u",r.deal_ticket),(int)r.broker_return_code,r.fill_confirmed?"YES":"NO",r.exit_reason_code);}

void AttemptEntry(V29Leg &leg)
  {
   ...
   for(int i=0;i<7;i++)
     {
      ...
      SolTradePositionReport report;g_pm[i].ProcessClose(signal,SOLTRADE_CLOSE_STRATEGY,g_account[i],g_market[i],report);if(report.evaluated){if(report.broker_accepted)g_last_exit_submission_tick=TimeCurrent();Event(report.event_type,report.reason_code,SYMBOLS[i]+";"+report.reason);RecordExit("EXIT_ATTEMPT",i,report);}g_pm[i].GetStatus(g_position[i]);
     }
  }

void ProcessFinalCohortExit(datetime now)
  {
   if(g_final_exit_target<=0||now<g_final_exit_target)return;
   if(now>=g_final_exit_target+5*60)
     {
      int remaining=PortfolioPositions();if(remaining>0)g_missed+=remaining;
      g_final_exit_target=0;Event("FINAL_EXIT_WINDOW_MISSED","EXIT_WINDOW_MISSED",IntegerToString(remaining));return;
     }
   if(PortfolioPositions()>0&&g_close_target!=g_final_exit_target){g_close_target=g_final_exit_target;ClosePortfolio(g_final_exit_target);}
   if(PortfolioPositions()==0){g_final_exit_target=0;g_close_target=0;}
  }

void ProcessSchedule(datetime now)
  {
   if(g_next>=g_leg_count){ProcessFinalCohortExit(now);return;}datetime target=g_legs[g_next].target;if(now<target)return;
   ...
   RefreshAll();datetime next_exit_target=g_legs[g_next].exit_time;
   for(int j=0;j<4;j++)AttemptEntry(g_legs[g_next+j]);
   g_next+=4;g_rebalances++;g_close_target=0;g_final_exit_target=next_exit_target;
  }
...
bool WriteDeals()
  {
   ...
   for(int i=0;i<HistoryDealsTotal();i++){ulong deal=HistoryDealGetTicket(i);if(deal==0)continue;long magic=HistoryDealGetInteger(deal,DEAL_MAGIC);if(magic<=V29_MAGIC_BASE||magic>V29_MAGIC_BASE+7)continue;datetime time=(datetime)HistoryDealGetInteger(deal,DEAL_TIME);FileWrite(h,StringFormat("%I64u",deal),StringFormat("%I64u",(ulong)HistoryDealGetInteger(deal,DEAL_ORDER)),StringFormat("%I64u",(ulong)HistoryDealGetInteger(deal,DEAL_POSITION_ID)),TS(time),StringFormat("%I64d",HistoryDealGetInteger(deal,DEAL_TIME_MSC)),(int)HistoryDealGetInteger(deal,DEAL_ENTRY),(int)HistoryDealGetInteger(deal,DEAL_TYPE),(int)HistoryDealGetInteger(deal,DEAL_REASON),DoubleToString(HistoryDealGetDouble(deal,DEAL_VOLUME),8),DoubleToString(HistoryDealGetDouble(deal,DEAL_PRICE),10),DoubleToString(HistoryDealGetDouble(deal,DEAL_PROFIT),8),DoubleToString(HistoryDealGetDouble(deal,DEAL_COMMISSION),8),DoubleToString(HistoryDealGetDouble(deal,DEAL_SWAP),8),DoubleToString(HistoryDealGetDouble(deal,DEAL_FEE),8),magic,HistoryDealGetString(deal,DEAL_SYMBOL),HistoryDealGetString(deal,DEAL_COMMENT),(time>=EligibleFrom&&time<EligibleTo)?"YES":"NO");}FileClose(h);return true;
  }

bool WriteSummary()
  {
   int open=PortfolioPositions();int h=FileOpen(OutputRoot+"\\run-summary.csv",FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON,',');if(h==INVALID_HANDLE)return false;FileWrite(h,"field","value");FileWrite(h,"schema","SOLTRADE_PHASE6_V29_RUN_SUMMARY_V1");FileWrite(h,"execution_instance_id",ExecutionInstanceId);FileWrite(h,"dataset",DatasetId);FileWrite(h,"schedule_dataset",ScheduleDatasetId);FileWrite(h,"execution_layer",ExecutionLayer);FileWrite(h,"schedule_legs",g_leg_count);FileWrite(h,"processed_rebalances",g_rebalances);FileWrite(h,"entry_attempts",g_entry_attempts);FileWrite(h,"entry_fills",g_entry_fills);FileWrite(h,"exit_fills",g_exit_fills);FileWrite(h,"missed_legs",g_missed);FileWrite(h,"spread_blocks",g_spread_blocks);FileWrite(h,"risk_blocks",g_risk_blocks);FileWrite(h,"execution_blocks",g_execution_blocks);FileWrite(h,"max_portfolio_positions",g_max_portfolio_positions);FileWrite(h,"final_exit_target_cleared",g_final_exit_target==0?"YES":"NO");FileWrite(h,"open_positions_at_end",open);FileWrite(h,"max_tick",TS(g_max_tick));FileWrite(h,"seal_breach",g_seal_breach?"YES":"NO");bool valid=g_preflight&&!g_seal_breach&&g_leg_count==ExpectedScheduleLegs&&g_next==g_leg_count&&g_final_exit_target==0&&g_max_portfolio_positions<=4&&open==0;FileWrite(h,"run_evidence_status",valid?"PASS":"FAIL");FileClose(h);return valid;
  }

int OnInit()
  {
   ArrayResize(g_legs,0);string reason="";if(!Preflight(reason)){Print("SOLTRADE_V29_PREFLIGHT_FAILED | ",reason);return INIT_PARAMETERS_INCORRECT;}
   ...
   for(int i=0;i<7;i++){ResetSolTradeMarketSnapshot(g_market[i]);ResetSolTradeExecutionStatus(g_execution[i]);ResetSolTradePositionStatus(g_position[i]);EvaluateSolTradeAccountSafety(g_config[i],g_account[i]);datetime last=0;if(!InitialiseSolTradeMarketData(g_config[i],last,reason)||!g_exec[i].Initialise(g_config[i],g_account[i].account_identifier_hash,reason)||!g_pm[i].Initialise(g_config[i],g_account[i].account_identifier_hash,reason)){Print("SOLTRADE_V29_ENGINE_INIT_FAILED | ",SYMBOLS[i]," | ",reason);return INIT_FAILED;}g_atr[i]=iATR(SYMBOLS[i],PERIOD_D1,14);if(g_atr[i]==INVALID_HANDLE)return INIT_FAILED;}
   if(!g_risk_engine.Initialise(g_config[0],g_account[0].account_identifier_hash,TimeCurrent(),AccountInfoDouble(ACCOUNT_EQUITY),reason)){Print("SOLTRADE_V29_RISK_INIT_FAILED | ",reason);return INIT_FAILED;}ResetSolTradeRiskStatus(g_risk);g_preflight=true;Event("PREFLIGHT_PASSED","NONE","tester_only=YES;optimization=NO;symbols=7");return INIT_SUCCEEDED;
  }

void OnTick(){datetime now=TimeCurrent();if(now>=ResearchCutoff){g_seal_breach=true;return;}if(now>=EligibleTo)return;g_max_tick=now;RefreshAll();ProcessSchedule(now);g_max_portfolio_positions=MathMax(g_max_portfolio_positions,PortfolioPositions());}
   ...
   for(int i=0;i<7;i++)
     {
      ...
      SolTradePositionReport exit;if(g_pm[i].HandleTradeTransaction(t,exit)){if(exit.fill_confirmed)g_exit_fills++;Event(exit.event_type,exit.reason_code,SYMBOLS[i]+";"+exit.exit_reason_code);RecordExit("EXIT_TRANSACTION",i,exit);string reason="";g_risk_engine.RecordClosedOutcome("V29_EXIT_"+StringFormat("%I64u",exit.deal_ticket),exit.final_profit_loss,TimeCurrent(),AccountInfoDouble(ACCOUNT_EQUITY),reason);g_risk_engine.GetStatus(g_risk);}
     }
  }
double OnTester(){bool ok=WriteDeals()&&WriteSummary();if(g_events!=INVALID_HANDLE)FileFlush(g_events);if(g_transactions!=INVALID_HANDLE)FileFlush(g_transactions);Print("SOLTRADE_V29_RUN_RESULT | status=",ok?"PASS":"FAIL"," | dataset=",DatasetId," | layer=",ExecutionLayer," | entries=",g_entry_fills," | exits=",g_exit_fills," | open=",PortfolioPositions());return ok?1.0:0.0;}
